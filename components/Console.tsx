...
function ConsoleFeature({ icon: Icon, title, desc, color, onClick }: ConsoleFeatureProps) {
    return (
        <div
            onClick={onClick}
...
function ThreatItem({ filename, method, severity, score, timestamp }: ThreatItemProps) {
    const config = severityConfig[severity];

    return (
        <div className="group hover:bg-white/5 p-2 rounded-lg transition-all border border-transparent hover:border-white/10 cursor-pointer">
                ...
                <GlassCard className="lg:col-span-2 min-h-[400px] relative overflow-hidden flex flex-col border-white/10 bg-black/50">
                    ...
                    <div className="mt-12 p-6 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30 relative overflow-hidden group hover:bg-red-500/20 transition-all cursor-pointer">
                            ...
                            <div className="p-4 rounded-lg bg-neon-blue/10 border border-neon-blue/30 relative overflow-hidden group hover:bg-neon-blue/15 transition-all cursor-pointer">
                        ...
                        <div className="rounded-lg bg-black/40 border border-white/10 overflow-hidden">
                            ...
                            <div className="p-4 space-y-3 max-h-56 overflow-y-auto">
                                <ThreatItem filename="CobaltStrike_Beacon.dll" method="MemScan" severity="critical" score={95} timestamp="2m ago" />
                                <ThreatItem filename="mimikatz.exe" method="CredDump" severity="critical" score={98} timestamp="5m ago" />
                                <ThreatItem filename="powershell_obfuscated.ps1" method="Heuristic" severity="high" score={78} timestamp="12m ago" />
                                <ThreatItem filename="unknown_packer.exe" method="ML_Engine" severity="medium" score={62} timestamp="18m ago" />
                                <ThreatItem filename="suspicious_macro.docm" method="Static" severity="low" score={45} timestamp="25m ago" />
                            </div>

                            <div className="px-4 py-2 bg-white/5 border-t border-white/10 flex items-center justify-between">
                                <div className="text-[10px] text-gray-500 font-mono">247 detections in last 24h</div>
                                <div className="text-[10px] text-neon-blue font-mono hover:text-neon-blue/80 cursor-pointer transition-colors">View All →</div>
                            </div>
                        </div>
                    </div>
                </GlassCard>
